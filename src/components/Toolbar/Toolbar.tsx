...
function Toolbar() {
  ...
  const setLight = (light: Number) => {
    sessionStorage.setItem("light", light.toString());
    setParams({ ...params, light });
  };
  const setVolume = (volume: Number) => setParams({ ...params, volume });
  const setAnimation = () => {
    sessionStorage.setItem("animation", (!params.animation).toString());
    setParams({ ...params, animation: !params.animation });
  };
  const setColor = () => {
    sessionStorage.setItem("color", (!params.color).toString());
    setParams({ ...params, color: !params.color });
  };
  const setFullscreen = () => {
    sessionStorage.setItem("fullscreen", (!params.fullscreen).toString());
    setParams({ ...params, fullscreen: !params.fullscreen });
  };
  const toggleSlideshow = () =>
    setParams({ ...params, slideshow: !params.slideshow });
  ...
  const setFavorite = () =>
    setParams({ ...params, favorites: toggleIndexFavorite() });
  const setMood = () => {
    sessionStorage.setItem("mood", (!params.mood).toString());
    setParams({ ...params, mood: !params.mood });
  };

  const toggleIndexFavorite = () => {
    //Legge til movieIndex hvis den eksisterer i params.favorites,
    // Eller fjerne den fra lista om den allerede eksisterer.
    let filteredArray: number[] = [];
    if (params.favorites.includes(params.posterIndex)) {
      //Remove if movie already in favorites
      filteredArray = params.favorites.filter(
        (movie) => movie !== params.posterIndex
      );
    } else {
      //Hvis den ikke er i lista, gjør den til favoritt.
  ...
  const slideshow = () => {
    ...
    if (!params.slideshow) {
      const id = setInterval(
        () => setSlideshowBar((prev) => (prev + 0.1) % 15),
        100
      );
  ...
  if (params.loading) {
    return (
      <div
        className={
          params.fullscreen ? styles.ToolbarFullscreen : styles.Toolbar
        }
      ></div>
    );
  }
  ...
  return (
    <div style={{ display: "unset" }}>
      ...
      >
        ...
        <div>
          ...
          <div className={styles.Group}>
            ...
            <div
              onClick={() => params.light > 40 && setLight(params.light - 4)}
            >
              -
            </div>
          </div>
        ...
        <div>
          ...
          <div className={styles.Group}>
            ...
            <div
              onClick={() => params.volume > 0 && setVolume(params.volume - 1)}
            >
              -
            </div>
          </div>
        ...
        {/* Animasjoner */}
        <div
          className={params.animation ? styles.ButtonActive : styles.Button}
        ...
        {/* Sort-hvitt */}
        <div
          className={params.color ? styles.ButtonActive : styles.Button}
          onClick={setColor}
        >
          Fargefilm
        </div>
        ...
        {/* Fullskjerm */}
        <div
          className={params.fullscreen ? styles.ButtonActive : styles.Button}
        ...
        <div className={styles.ButtonBar} onClick={slideshow}>
          <div
            ...
            style={{ width: `${(slideshowBar / 15) * 100}%` }}
          >
        ...
        <div className={styles.Row}>
          <div className={styles.Toggle} onClick={setFavorite}>
            {params.favorites.includes(params.posterIndex) ? "Fjern" : "Lagre"}{" "}
            favoritt
          </div>
          {/* Favoritter */}
          {params.fullscreen ? null : (
            <div className={styles.Toggle} onClick={toggleFavorites}>
              {params.showFavorites ? "Se alle" : "Se favoritter"}
            </div>
          )}
        </div>
      </div>
      <div
        ...
        style={{ opacity: params.light < 55 ? 0 : 1 }}
      >
