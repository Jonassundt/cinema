...
function Poster() {
  ...
  const height = params.fullscreen ? 1200 : 800;
  ...
  const prevPoster = () => {
    ...
    setTimeout(
      () =>
        setPosterIndex(
          (params.posterIndex + posters.length - 1) % posters.length
        ),
      400
    ); //Ensures that modulo works correctly.
    setTimeout(() => setLeftAnimation(false), 800);
    sessionStorage.setItem(
      "posterIndex",
      ((params.posterIndex + posters.length - 1) % posters.length).toString()
    );
  };
  ...
  const nextPoster = (slideshowIndex: number = -1) => {
    ...
    setTimeout(
      () =>
        slideshowIndex >= 0
          ? setPosterIndex(slideshowIndex)
          : setPosterIndex((params.posterIndex + 1) % posters.length),
      400
    );
    setTimeout(() => setRightAnimation(false), 800);
    sessionStorage.setItem(
      "posterIndex",
      ((params.posterIndex + 1) % posters.length).toString()
    );
  };
  ...
  const startSlideshow = () => {
    ...
    const id = setInterval(() => {
      setSlideshowIndex((prev) => (prev + 1) % posters.length);
    }, 15000);
  ...
  useEffect(() => {
    ...
    } else if (params.slideshow && !slideshowId) {
      //hvis det ikke kjøres slideshow nå, og slideshow-variabel er sann, start slideshow
      startSlideshow();
    } else if (!params.slideshow && slideshowId) {
      //hvis det kjøres slideshow nå, og slideshow-variabel er false -> stopp slideshow
      stopSlideshow();
  ...
  if (params.loading) {
    return (
      ...
      <div>
        <div
          className={
            params.fullscreen ? styles.PosterFullscreen : styles.Poster
          }
        >
        ...
        {params.fullscreen ? null : (
          <div
            className={
              params.mood ? styles.Description : styles.DescriptionLight
            }
          >
  ...
  return (
    ...
    <div>
      ...
      {params.fullscreen ? null : (
        <div
          className={params.mood ? styles.Description : styles.DescriptionLight}
        >
